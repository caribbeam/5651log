...
def device_edit(request, company_slug, device_id):
    ...
    if request.method == 'POST':
        ...
        device.cpu_usage = float(request.POST.get('cpu_usage', 0))
        device.memory_usage = float(request.POST.get('memory_usage', 0))
        device.temperature = float(request.POST.get('temperature')) if request.POST.get('temperature') else None
        device.uptime = request.POST.get('uptime') if request.POST.get('uptime') else None
